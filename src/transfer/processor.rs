    ...
    B: AnkiPackageBuilderTrait,
{
    ...
    pub fn write_to_file<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        self.builder.write_to_file(path).map_err(DuoloadError::from)
    }
}
...
mod tests {
    ...
    impl crate::duocards::DuocardsClientTrait for TestDuocardsClient {
        async fn fetch_page(&self, _deck_id: &str, _cursor: Option<String>) -> Result<DuocardsResponse> {
    ...
    fn create_test_response(cards: Vec<VocabularyCard>, has_next_page: bool, end_cursor: Option<String>) -> DuocardsResponse {
        let card_edges: Vec<CardEdge> = cards.into_iter().map(|card| {
            CardEdge {
                node: Card {
